...
public class AbChinaBillHandler : BillHandlerBase
{
    public override FeideeBill GetFeideeBill(Stream stream)
    {
        var excelFile = new ExcelMapper(stream) { HeaderRowNumber = 2, MinRowNumber = 2 };
            ...
            else
            {
                foreach (var item in group)
                {
                    ...
                    exportTemplate.Transfer = item
                        .Where(x => x.TradingSummary == "支付宝" || x.TradingSummary == "财付通" ||
                                    x.TradingSummary == "微信支付").Select(x =>
                            new FeideeBillItem
                            {
                                TransactionDateTime =
                                    GetTransactionDateTime(x.TransactionDate, x.TransactionTime),
                                SourceAccount = "中国农业银行",
                                TargetAccount = BillHelper.GetTargetAccount(x.TradingSummary),
