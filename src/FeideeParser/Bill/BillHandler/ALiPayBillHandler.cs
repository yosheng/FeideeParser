...
public class ALiPayBillHandler : BillHandlerBase
{
    public override FeideeBill GetFeideeBill(Stream stream)
    {
        ...
        foreach (var group in groupDataList)
        {
            ...
            if (group.Key.Equals("收入"))
            {
                ...
                {
                    exportTemplate.Income.AddRange(item.Select(x => new FeideeBillItem
                    {
            ...
            if (group.Key.Equals("其他") || group.Key.Equals("不计收支"))
            {
                foreach (var item in group)
                {
                    exportTemplate.Income.AddRange(item.Where(x => x.TransactionStatus.Equals("退款成功")).Select(x =>
                        new FeideeBillItem
                        {
                            TransactionDateTime = GetTransactionDateTime(x.TransactionTime),
                            Category = "职业收入",
                    ...
                    exportTemplate.Income.AddRange(item.Where(x => x.TransactionStatus.Equals("交易成功")).Select(x =>
                        new FeideeBillItem
                        {
                            TransactionDateTime = GetTransactionDateTime(x.TransactionTime),
                            Category = "职业收入",
